...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Model generalizuje stosunkowo dobrze, ponieważ różnica między RMSE dla zbioru treningowego a testowego jest niewielka. W kontekście cen domów błąd uzyskany przez model jest na akceptowalnym poziomie.
...
ridge_alphas = np.linspace(0.1, 100, num = 1000)
reg_ridge = RidgeCV(alphas = ridge_alphas)
reg_lasso = LassoCV(n_alphas=1000, cv=5, random_state=0)

reg_ridge.fit(X_train, y_train)
reg_lasso.fit(X_train, y_train)

reg_ridge_alpha = round(reg_ridge.alpha_, 4)
reg_lasso_alpha = round(reg_lasso.alpha_, 4)
##pred
reg_ridge_train_y_pred = reg_ridge.predict(X_train)
reg_ridge_test_y_pred = reg_ridge.predict(X_test)

reg_lasso_train_y_pred = reg_lasso.predict(X_train)
reg_lasso_test_y_pred = reg_lasso.predict(X_test)
##exp
reg_lasso_train_y_pred_exp = np.exp(reg_lasso_train_y_pred)
reg_lasso_test_y_pred_exp = np.exp(reg_lasso_test_y_pred)

reg_ridge_train_y_pred_exp = np.exp(reg_ridge_train_y_pred)
reg_ridge_test_y_pred_exp = np.exp(reg_ridge_test_y_pred)

y_train_exp = np.expm1(y_train)
y_test_exp = np.expm1(y_test)

##rmse
lasso_train_rmse = root_mean_squared_error(y_train_exp, reg_lasso_train_y_pred_exp)
print(lasso_train_rmse)
lasso_test_rmse = root_mean_squared_error(y_test_exp, reg_lasso_test_y_pred_exp)
print(lasso_test_rmse)
ridge_train_rmse = root_mean_squared_error(y_train_exp, reg_ridge_train_y_pred_exp)
print(ridge_train_rmse)
ridge_test_rmse = root_mean_squared_error(y_test_exp, reg_ridge_test_y_pred_exp)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Najlepsze wyniki osiągnęliśmy dla modelu Lasso. Różnica między wynikami dla zbioru treningowego a testowego jest minimalna, co sugeruje, że udało się zminimalizować przeuczenie modelu.
...
plt.text(0, value_counts[0], str(value_counts[0]), ha='center', va='bottom')
plt.text(1, value_counts[1], str(value_counts[1]), ha='center', va='bottom')
...
#No Regularization
nol = LogisticRegression(penalty=None, class_weight="balanced", random_state=0)
...
print(f"nol_precision: {nol_precision:.4f}")
print(f"nol_recall: {nol_recall:.4f}")
print(f"nol_f1: {nol_f1:.4f}")
...
print(f"l1_precision: {l1_precision:.4f}")
print(f"l1_recall: {l1_recall:.4f}")
print(f"l1_f1: {l1_f1:.4f}")
...
print(f"l2_precision: {l2_precision:.4f}")
print(f"l2_recall: {l2_recall:.4f}")
print(f"l2_f1: {l2_f1:.4f}")
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
nol_y_train_pred = nol.predict(X_train)
nol_y_test_pred = nol.predict(X_test)

f1_train = f1_score(y_train, nol_y_train_pred)
f1_test = f1_score(y_test, nol_y_test_pred)

print(f"f1 for test: {f1_test:.4f}")
print(f"f1 for train: {f1_train:.4f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Metryka f1 zarówno dla tanych testowych jak i treningowych zwraca zbliżone wartości dla tego modelu, nawet bez regularyzacji. Oznacza to, że overfitting nie występuje. Regularyzacja ma znikomy wpływ na wartość metryki, możemy zatem ją pominąć. Według mnie ważniejsze w tym przypadku jest precisision, poniważ zależy nam na tym, abyśmy klase pozytywną rozpoznawali maksymalnie dokładnie. Aby zmienić stosunek precision do recolla, mozna zmienić balans zbioru, tak zby było w nim więcej próbek należących do klasy pozytywnej. 
...
nol = LogisticRegression(penalty=None, class_weight="balanced", random_state=0, max_iter=1000)

nol.fit(X_train, y_train)

y_train_pred = nol.predict(X_train)
y_test_pred = nol.predict(X_test)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Nie ma sensu przeprowadzać regulazryzacji, ponieważ model nie jest jest przeuczony, co możemy zobserwować po tym że metryka f1 dla zbioru testowego i zbioru traningowego zwraca podobne wartości.
